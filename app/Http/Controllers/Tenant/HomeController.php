...
final class HomeController
{
    ...
    private function courses()
    {
        ...
        /*
         ...
         |
         | بطاقة الكورس تعرض مدرّسه وتصنيفه ومرحلته؛ وستّ بطاقاتٍ بلا
         | تحميلٍ مسبق تعني ثمانية عشر استعلاماً زائداً في الصفحة الأولى
         | التي يراها الزائر — وهي أهمّ صفحةٍ في الأداء.
         |
         | والمرحلة ليست زينة: ثيمات المدرّسين تضعها شارةً على الغلاف،
         | وتحميلها الكسول ممنوعٌ في هذا التطبيق فتسقط الصفحة كلها.
         */
    ...
    private function groups()
    {
        ...
        /*
         | «مفتوحة» بتعريف النموذج لا بتعريف مكتوب هنا.
         |
         | `scopeOpen` يعني `open` و`running` معاً — والشرط المكتوب هنا
         | كان يعني `open` وحدها. والبذرة تُنشئ مجموعات السنة الدراسية
         | `running`، فيفتح المدرّس موقعه بعد أسبوع من بدء الترم فيجد
         | قسم المجموعات اختفى كلّه، ومجموعاته تعمل وفيها مقاعد.
         |
         | ومن بدأ ترمُه ما زال يقبل طالباً جديداً: المقاعد الفارغة هي
         | ما يقرّر، لا تاريخ أول حصة.
         |
         | و`branch` يُحمَّل معها: `venueLabel()` يسقط إليه حين لا يكون
         | المكان أونلاين ولا بيتاً ولا سنتراً، والتحميل الكسول ممنوع —
         | فبطاقة مجموعةٍ في فرعٍ كانت تُسقط الصفحة الرئيسية كلها.
         */
        return Group::open()
            ->with(['subject', 'grade', 'schedules', 'branch'])
            ->limit(6)->get();
    }
