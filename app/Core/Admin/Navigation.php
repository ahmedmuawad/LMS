    ...
    public function groups(): array
    {
        ...
        foreach (config('admin-navigation.groups') as $group) {
            ...
            foreach ($group['items'] as $item) {
                ...
                /*
                 | شاشة يحلّ محلّها موديول آخر حين يُفعَّل: «طلاب الكورسات»
                 | تُخفى حين تكون إدارة الحصص هي مصدر الطلاب — وإلا ظهر
                 | في القائمة عنوانان لشيء واحد، أحدهما فارغ دائماً.
                 */
                $unless = $item['unless_module'] ?? null;

                if ($unless !== null && $this->moduleEnabled($unless)) {
                    continue;
                }
